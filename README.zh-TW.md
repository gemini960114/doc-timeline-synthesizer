...
![殘餘錯誤定性分析與分類](docs/images/fig4_failure_attribution.png)

針對分層雙庫 RAG（Condition 2）在嚴格字串比對下之 15 題未匹配案例進行事後定性錯誤分析（Post-hoc Error Analysis），分類結果如下：

| 錯誤成因分類 (Failure Category) | 題數 ($N=15$) | 佔比 | 事實意圖判定 | 根因說明 |
|:---|:---:|:---:|:---:|:---|
| **表面格式與語法差異 (Surface-Form Variations)** | **13** | **86.7%** | 事實意圖一致 | 模型正確推論出權威數值與決策事實，因格式表現形式（如條列式排版、中文大寫金額寫法、日期標點符號）未完全符合嚴格前綴匹配。 |
| **檢索切塊截斷 (Retrieval Context Truncation)** | 1 | 6.7% | 細節截斷遺漏 | Top-5 SSoT 切塊包含總額與主要分項，因切塊長度限制遺漏單一孤立細項。 |
| **評分標準題目歧義 (Benchmark Rubric Ambiguity)** | 1 | 6.7% | 題幹與標準答案不對稱 | 題幹詢問條件限制，標準答案要求未在題幹中提示之特定預算金額。 |
| **過期草案陷阱 (Superseded Draft Traps)** | **0** | **0.0%** | **在評測樣本中未觀察到** | 在本次評測的 15 筆未匹配樣本中，未觀察到採信過期公文或作廢草案之案例。 |

此分析顯示 85.0% 是在嚴格正則與字串抽取下的重現性基準指標；絕大多數非匹配案例屬於自然語言表面排版差異，而非事實推論錯誤或時序版本錯置。
