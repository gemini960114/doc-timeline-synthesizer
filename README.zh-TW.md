...
![殘餘錯誤剖析與潛在語意正確率](docs/images/fig4_failure_attribution.png)

針對分層雙庫 RAG（Condition 2）剩餘 15%（15 題）未通過嚴格字串比對之題目進行逐題對抗性稽核，結果顯示 85.0% 確定性分數為保守下界：

| 錯誤成因分類 (Root Cause) | 題數 ($N=15$) | 佔比 | 語意事實判讀結果 | 根因說明 |
|:---|:---:|:---:|:---:|:---|
| **評測 Harness 字串比對偽陰性** | **13** | **86.7%** | **100% 事實推論正確** | 模型已正確推論出權威數值與決策事實，僅因格式表現形式（如條列式 Markdown 排版、中文大寫金額轉換、日期標點差異）未能完全符合評測腳本之嚴格前綴匹配。 |
| **檢索切塊截斷 (Retrieval Cutoff)** | 1 | 6.7% | 真實檢索遺漏細節 | Top-5 SSoT 切塊包含總額與主要分項，但因切塊長度限制遺漏了單一孤立細項。 |
| **評分標準題目歧義 (Rubric Mismatch)** | 1 | 6.7% | 標準答案定義不對稱 | 題目詢問條件限制，但標準答案要求未在題幹中提及之特定預算金額。 |
| **致命性過期草案陷阱 (Catastrophic Traps)** | **0** | **0.0%** | **零採納 (0 陷阱)** | 在所有 15 題中，模型均未採信已被時序覆蓋之過期公文或非正式草案。 |

經語意與事實嚴格核驗，分層雙庫架構之實質**潛在語意決策正確率達 98.0%（98/100）**，且完全杜絕了誤採過期作廢數據的致命性風險。
