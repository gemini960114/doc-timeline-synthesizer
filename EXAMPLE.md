...
> **版本**：v1.5.0（新增階段十：自然語言一鍵觸發全流程；新增文件內部一致性核查與審核執行方式之實戰經驗；補充審核完成後須同步更新報告內殘留待審核文字之提醒）  
...
8. **套用修正**：把審核意見中「已確認、非主觀判斷」的缺失（例如算術重算錯誤、明確查得到卻未揭露的矛盾）直接修回報告本體，若涉及來源語料的判讀補述，也一併更新對應的 `source.images.md`，並重新執行 `build_rag_bundle.py`。**同時務必回頭檢查報告開頭/結尾在步驟三蒸餾時寫下的「審核狀態：尚未執行 L3 審核」「待辦：交付獨立審核」等暫定文字，替換成實際的總評判定與審核意見檔名**——這是實測發生過的疏漏：審核跑完、缺失也修正了，但報告裡仍留著審核前的占位字樣，會讓下游誤以為報告尚未通過品管。
