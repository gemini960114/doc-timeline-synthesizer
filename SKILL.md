...
【doc-timeline-auditor】(L3，獨立 Agent／新 Session) ── 逆向核對 Citation、來源風險與跨時間戳拼接，產出《審核意見.md》
      │
      ▼
【下游應用】 ── 高精準 RAG 知識庫 / NotebookLM / 決策簡報（僅限 L3 判定 PASS 或 PASS WITH CAVEATS 之報告）
```

> [!IMPORTANT]
> L3 審核**不是** L2 蒸餾流程步驟四「抽樣回溯驗證」的替代品，兩者都要做。步驟四是蒸餾 Agent 對自己產出的初步自檢；L3 是換一個沒看過蒸餾過程的全新 Agent／Session，對成品報告做對抗性複核。高風險或高關注度的報告（例如將送交決策層或對外發布），應在下游使用前完成 L3 審核，詳見 [`doc-timeline-auditor/SKILL.md`](doc-timeline-auditor/SKILL.md)。
...
為避免不同業務領域（如「財務紀錄」與「技術規格」）互相干擾造成檢索污染，建議採取專題目錄分流：

```text
data/ （原始檔案分類放置）
├── domain_a/   <--- 同一業務範圍的財務與執行紀錄
├── domain_b/   <--- 同一業務範圍的行政與治理紀錄
└── domain_c/   <--- 同一業務範圍的計畫與技術紀錄

output/ （各自獨立累積與蒸餾）
├── domain_a/   ---> 產出專屬【domain_a_總整理.md】
├── domain_b/   ---> 產出專屬【domain_b_總整理.md】
└── domain_c/   ---> 產出專屬【domain_c_總整理.md】
```

### 領域隔離原則：
1. **業務解耦**：每個領域只處理自身的實體、指標與責任範圍，避免跨領域語意污染。
...
自動忽略下列無決策價值的樣板行政文字：
- 文件抬頭、收發文號、流程性文字、聯絡資訊。
- 每一份文件開頭重複的背景宣示（例如「因應國際科技發展趨勢...」）。
...
與下方「禁止跨時間戳拼接」不同，這是**同一份文件內部**的矛盾：敘述性文字（例如自由文字段落或會議摘要）與結構化欄位（表格、勾選框、圖片判讀內容）可能互相衝突。
- **先揭露衝突，不預設格式勝負**：不可因資料是表格或敘述文字就自動判定真偽。必須依出處鏈、文件狀態（草稿／核定）、時間有效性、欄位語意與其他可核對證據判定權威性。
- 若結構化欄位是正式核定欄位，且敘述文字有可驗證的範本沿用或未更新跡象，可以在該語料範圍內優先採信結構化欄位；此判斷不得升格為通用規則。
- **不可只採信一方就悄悄略過**：Citation欄位必須明確記錄衝突、採信結果、理由與仍存在的不確定性。若證據不足，保留為未解衝突，不強制產生單一答案。

### 4. 範圍感知的時序與權威仲裁（Scope-Aware Temporal and Authority Arbitration）
- **先比對範圍，再比較新舊**：只有在實體、屬性、目標年度／期間、地理或計畫範圍、單位與量測語意一致時，才把不同數值視為同一事實的版本候選。
- **新近性只是仲裁因素之一**：在可比較的候選集合內，同時評估文件時間戳、草稿／核定狀態、發布或簽核權限、出處鏈與欄位語意；不得把「最新時間戳」等同於「必然正確」。
- **單一真相須有條件**：僅在證據足以支持時選定目前有效值；若問題詢問歷史年度，保留該年度值；若範圍不同或權威性無法判定，並列候選與差異原因，不強制覆寫。
- **出處標註（Provenance / Citation）**：每個關鍵數字均須標註來源檔名、日期、章節／表號及選用理由，便於人工抽查。
- **沿革保留**：確定被後續同範圍核定版本取代的舊提法，保留為決策演進脈絡（History），不與現行值混作同一狀態。
- **禁止跨時間戳拼接（Anti-Splicing Rule）**：同一主張若由多個計量單位共同描述，必須確認各單位的時間與範圍相容；新文件只更新部分單位時，標註未更新部分，不得用舊值冒充同期資料。
...
> [!NOTE]
> **流程層級說明**：本節描述 L2 蒸餾 Agent 在 L1 轉換完成後應執行的內部步驟；README.md 則提供操作者的端到端 quick start。跨文件對照時應以步驟名稱與輸入／輸出為準。

### 步驟零：圖片佔位符補強（選用，語料庫一次性前處理）
`docling-skill` 目前預設**未啟用**圖片內容描述（`do_picture_description`），遇到內嵌圖片時，只會將圖片以 base64 存入 `source.evidence.json`，並在 `source.md` 留下穩定佔位符 `[[image:picture-p0-0]]`——這不代表圖片內容無法辨識，只是尚未有任何 Agent 去看過。若略過此步驟，圖片裡的資訊（尤其掃描表單、手寫欄位、圖表化的統計數字）會在後續蒸餾與稽核時完全消失於文字檢索範圍之外。
...
> [!NOTE]
> 圖片不一定是無決策價值的雜訊。常見情境：表單可能以整頁掃描圖呈現，金額、項目名稱等關鍵欄位可能只存在圖片裡、未被 OCR 進正文——若略過本步驟，步驟三蒸餾與步驟四回溯驗證都可能把這類數字誤判為「查無出處」。
...
> [!IMPORTANT]
> `chronological_inventory.md` 之「關鍵指標初篩片段」僅為每份文件開頭數百字的截斷預覽，**不可作為蒸餾之唯一依據**。對字元數龐大（如超過 5 萬字）之 `source.md`（常見於大型計畫書、技術文件或彙整表），Agent 必須完整讀畢全文，逐一核對文件中所有指標版本，才能執行步驟四。僅依賴片段摘要進行蒸餾，是導致「新舊數值混拼」與「指標漏抓」的主要成因。
...
報告完成步驟四後，對於將送交決策層或用於任何對外用途的產出，**另起一個全新 Agent／Session**，掛載 `doc-timeline-auditor` 技能執行審核，取得《審核意見.md》。判定為 `FAIL` 者退回步驟三重新蒸餾；`PASS` 或 `PASS WITH CAVEATS` 者才可交付下游使用。內部草稿或低風險用途可視情況省略此步驟，但仍建議保留步驟四的自我抽查。
