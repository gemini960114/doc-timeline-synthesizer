...
> **口徑一致性檢查（Scope Consistency Check）**：報告中若並列多個「看起來性質相近、單位相同」的加總型數字（例如「全國目標X MW」「單一計畫全期程Y MW」「跨計畫加總Z億元」），**不可假設它們可以互相加總或相減來驗證彼此**，必須先確認三件事再下結論：(1) **統計範圍是否相同**——地理範圍（如「南科＋沙崙」是否涵蓋中科機台）、計畫邊界（科目層級 vs 個別CP計畫編號）是否一致；(2) **時間切點是否對齊**——是否都指同一年度或同一期程終點，不可拿117年度數字加118年度數字去對比118年度目標；(3) 若加總後與報告聲稱的總數有落差，該落差是否有合理解釋（例如尚未揭露的後續成長量），並將此檢查與解釋明確寫入審核意見，而非放任讀者自行嘗試加總後產生誤解。實測案例：金字塔簡報並列「全國23MW（南科＋沙崙加總）」「晶創台灣方案13.95MW（117年，含中科0.8MW機台）」「大南方10.62MW（118年）」，直接相加得24.57MW，與23MW有落差且讀者容易誤判為算錯——實際上是統計範圍（是否含中科）與時間切點（117年 vs 118年）皆不同，並非算術錯誤，但报告原本未揭露此差異，經使用者提問才發現。
